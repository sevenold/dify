...
<Row>
  <Col>
    ...
    <Properties>
      ...
      <Property name='doc_type' type='array[string]' key='doc_type'>
        Document type filter, used to specify which types of documents to retrieve information from. Pass an array of strings, where each string represents a document type. If not passed or an empty array is passed, all document types will be searched. Default is empty array `[]`
      </Property>
      <Property name='history' type='array[object]' key='history'>
        Conversation history, used to provide dialogue context. Each object in the array contains the following fields:
        - `role` (string) Message role, possible values:
          - `user` User message
  ...
  <Col sticky>
    ...
    <CodeGroup title="Request" tag="POST" label="/chat-messages" targetCode={`curl -X POST '${props.appDetail.api_base_url}/chat-messages' \\\n--header 'Authorization: Bearer {api_key}' \\\n--header 'Content-Type: application/json' \\\n--data-raw '{\n    "inputs": ${JSON.stringify(props.inputs)},\n    "query": "What are the specs of the iPhone 13 Pro Max?",\n    "response_mode": "streaming",\n    "conversation_id": "",\n    "user": "abc-123",\n    "doc_type": ["product_manual", "tech_specs"],\n    "history": [\n      {"role": "user", "content": "Tell me about iPhone models"},\n      {"role": "assistant", "content": "I can help you with information about iPhone models. What would you like to know?"}\n    ],\n    "files": [\n      {\n        "type": "image",\n        "transfer_method": "remote_url",\n        "url": "https://cloud.dify.ai/logo/logo-site.png"\n      }\n    ]\n}'`}>
