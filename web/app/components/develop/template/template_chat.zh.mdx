  ...
  <Col>
    ...
    <Properties>
      ...
      <Property name='doc_type' type='array[string]' key='doc_type'>
        文档类型过滤，用于指定从哪些类型的文档中检索信息。传入一个字符串数组，每个字符串代表一个文档类型。如果不传或传空数组则检索所有类型文档。默认为空数组 `[]`
      </Property>
      <Property name='history' type='array[object]' key='history'>
        历史对话记录，用于提供对话上下文。数组中的每个对象包含以下字段：
        - `role` (string) 消息角色，可选值：
          - `user` 用户消息
          - `assistant` AI助手消息
        - `content` (string) 消息内容
        默认为空数组 `[]`
